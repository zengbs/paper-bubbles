...
{\color{red} Cite references for pc-scale jet candidates of Sgr A\*, e.g., \citep{Li, Zhu}.}
...
\end{figure*}
 ...
 can also be realized by oblique jets.\ {\color{red} KY: Please add brief discussion about (1) the surface of the bubbles, e.g., the simulated surface is not as smooth as the observed one). The instabilities at the bubble surface may be suppressed by the magnetic draping effect (Lyutikov 2006, Y12) if magnetic fields were included in the simulations. (2) the uniform gamma-ray intensity: The simulated gamma-ray intensity distribution shown in Figure 8 does not appear to be as uniform as originally found in Su et al. (2012). As discussed above, the gamma-ray intensity is slightly higher close to the Galactic plane due to the stronger radiation field at lower latitudes. However, this level of brightness variations is consistent with the later observational data of Ackermann et al. (2014) and Selig et al. (2015).}
...
{\color{red} KY: Please also add a section (either in the Discussion or Appendix) to discuss about the parameter variations. I think the figures you showed me are very important and informative.}

{\color{red} KY: Please add a Discussion section to discuss the cooling times of CR electrons and the requirement for re-acceleration. Emphasize that our modeled gamma-ray and microwave spectra assumed that the underlying CR spectrum is spatially uniform. Therefore our results imply that, whatever the true re-acceleration mechanism is, the CR spectrum has to be spatially uniform with a spectral index of 2.4 in order to fit the observed spectra. Please also include this in the abstract and the conclusions since this is an important point to make.}
