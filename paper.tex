...
\section{Methodology}
  ...
  The pyFC randomly generates dimensionless 3D scalar field $f_{\text{clumpy}}(\bold{x})$\ % that is clumpy and porous.
  ...
  Finally, we multiply $f_{\text{clumpy}}(\bold{x})$ and $\rho_{\text{isoDisk}}(z)$\
  to obtain the density of ISM disk $\rho_{\text{ismDisk}}(\bold{x})$. i.e.\
  $\displaystyle\rho_{\text{ismDisk}}(\bold{x}) =\
  f_{\text{clumpy}}(\bold{x}) \rho_{\text{isoDisk}}(z)$.

  The temperature in each cell of the disk is determined by pressure equilibrium with hot gas.\

  The porosity of the warm phase arises by imposing an upper temperature cutoff for the\
  existence of clouds at $T_{\text{crit}} = 10^{4}$ K, beyond which clouds
  are deemed thermally unstable.
...
$\mu_{\text{clumpy}}$                 & Mean of scalar field                      & 1.0                                  &   N/A                          \\
$\ddag$  $\sigma_{\text{clumpy}}$     & Dispersion of scalar field                & 2.2                                  & \citep{Federrath2010}          \\
$\beta$                               & Power law index                           & -5/3                                 &   N/A                          \\ \midrule
\end{tabular}
\begin{tablenotes}
      \raggedright
      \item  $\dagger$  $k_{\text{min}}=375.0$ leads to the size of an individual clump $\sim 100$ pc.
      \item    $\ddag$  $\sigma_{\text{clumpy}}=$ ....
    \end{tablenotes}
\end{table*}
