...
%  when the cell temperature beyond the critical value $T_{\text{crit}}$.
  ...
  The AMR base level is covered by $16\times16\times32$ root cells, refined progressively on the mid-plane $z=0$\
  based on the gradient of mass desnity.\
  The highest refinement level is 7 so that the clumps within the disk can be adequately\
  resolved by approximately 30 cells along their diameter, 20 pc.
