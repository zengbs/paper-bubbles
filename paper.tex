...
\section{Methodology}
  ...
  Moreover, as the clouds are considered to be thermally unstable\
  when the cloud temperature beyond\
  the critical temperature of the warm atomic phase.\
  We replace the cell density with\
  $\rho_{\text{ismDisk}}T_{\text{ismDisk}}/T_{\text{atmp}}$\
  for a cell temperature greater than the critical value $T_{\text{crit}}$.


  %Doing so, the porosity of the disk can arises,\
