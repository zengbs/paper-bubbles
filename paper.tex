...
\section{Methodology}
  ...
  can be derived from the other known parameters and pressure equilibrium condition\
  ...
  $f(\bold{x})$ with $\rho_{\text{isoDisk}}(z)$ over all cells within the disk:\
  $\displaystyle\rho_{\text{ismDisk}}(\bold{x}) =\
  f(\bold{x}) \rho_{\text{isoDisk}}(z)$.\
  Also, the pressure equilibrium condition implies that the temperature of disk is
  $\displaystyle T_{\text{ismDisk}}(\bold{x}) =\
  T_{\text{isoDisk}}(z)\rho_{\text{isoDisk}}(z)/\rho_{\text{ismDisk}}(\bold{x})$.

%  Moreover, since the clouds are considered to be thermally unstable\
%  when the cloud temperature beyond\
%  the critical temperature of the warm atomic phase,\
%  we replace the cell density as\
%  $\rho_{\text{ismDisk}}T_{\text{ismDisk}}/T_{\text{atmp}}$\
%  when the cell temperature beyond the critical value $T_{\text{crit}}$.
...
\subsection{Inclined jet injection}
  Recent paper \citep{Gallimore2006} has proposed that\
  ...
  Motivated by these observations, we simulate three different inclined angles
  $10^{\circ}$, $45^{\circ}$, and $80^{\circ}$ with respect to the Galactic plane to\
  release the caveat that\
  the jet direction must be perpendicular to the Galactic plane, and in particular to\
  investigate the effect of inclined angle on the shape of bubbles.


  The jet is constantly ejected from a cylindrical source with
  four-velocity


  the density contrast between the thermal gas contained in the jets
  and the ambient gas

  We resolve the jet source at the center of the simulation domain,\
