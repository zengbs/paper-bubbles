...
\section{Results}
  ...
  \begin{figure}
    ...
    \caption{
             ...
             the origin for a duration of 1.2 Myr under four different circumstances.\
             Comparison between the cases with a clumpy (first column from left)\
             and a smooth disk (second) in a stratified atmosphere\
             shows that the density distribution of the dense disk has insignificant effect\
             on the overall dynamics of the bubbles. However, the bubbles with the smooth disk (third)\
             in an uniform atmosphere is spherical-shape,\
             suggesting the stratification facilitates bubbles elongation significantly.\

             The first three and the rightmost columns shows that the under pressure, cold, and dense bubbles
             the bubbles without disk is tilted even\
             in the stratified atmosphere, and

             and the under pressure, cold, and dense bubbles (dashed box)\
             is also evident in the first three columns,

             indicating dense disk is a crucial component for axis-symmetric bubbles formation.\

             Comparing all cases show that the under pressure, cold, and dense bubbles (dashed box)\
             ionly appear if disk is present.

             The first three diagrams and the rightmost diagram show that the air bubbles can only be generated\
             with the disk and without the disk, the air bubbles will be skewed.
             }
...
follows out to a radius of 250 kpc the observed density profile of \citep{temperature-MW}.
  ...
  %\begin{figure*}[ht]
  \begin{figure*}
    \subfigure[Synthetic count rate (photons s$^{-1}$ deg$^{-2}$) in the 0.6-1.0 keV range,\
               using a Hammer-Aitoff projection. Throughout this paper we show sky maps in\
               Galactic coordinates centered on the Galactic center.]
     {
      \includegraphics[width=\linewidth]{figures/fig__xray_0.8keV_angle_000.png}
      \label{fig__xray_0.8keV_angle_000}
     }
    \subfigure[Comparison of observed (black; \citealt{Predehl2020}) and simulated (red) one-dimensional\
     ...
     {
      \includegraphics[width=\linewidth]{figures/fig__x-ray-profile-0.8keV-000.png}
