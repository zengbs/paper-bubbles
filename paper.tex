...
\section{Results}
  ...
  \begin{figure*}
    \includegraphics[width=\linewidth]{figures/fig__jetI5+ismSeed3-45deg_no-disk-stratified-ambient-Pres-Dens-Temp.png}
    \caption{
             Comparison between the case with the clumpy (the first column from the left)\
             and smooth disk (the second) in the stratified atmosphere\
             shows that the density distribution of the dense disk has insignificant effect\
             on the overall dynamics of the bubbles. However, the bubbles with the smooth disk (the third)\
             in a uniform atmosphere is spherical-shape,\
             suggesting the stratified ambient helps to elongate bubbles notably.\
             Moreover, the rightmost column shows that the bubbles without the disk is tilted even\
             in the stratified atmosphere, indicating the dense disk is a crucial component\
             for the formation of axis-symmetric bubbles.\
             In all the cases, the bipolar jets are injected at the origin along the $z=-y$ direction\
             ...
             % , undetected by any telescopes (frame the inner bubble)
             }
