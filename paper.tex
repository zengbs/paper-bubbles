...
\section{Methodology}
  ...
  We tabulate all parameters in \Cref{table-parameters}, except for $\rho_{\text{atmp}}^{\text{peak}}$ that\
  ...
  The initial density profile based on the above construction is shown in \Cref{density-profile}\
  ...
  the log-normal probability distribution with mean $\mu$ and dispersion $\sigma$,\
  %the log-normal probability distribution \citep{Berkhuijsen2008} with mean $\mu$ and dispersion $\sigma$,\
  ...
  $k_{\text{max}}$ is one-half of the cell numbers along the edge of the simulation box,\
  and $k_{\text{min}}$ is set to 375.0, corresponding to the maximum size of an individual cloud $\sim 20$ pc.\
  \Cref{table-parameters} presents the parameters of clumpy disk and their references.
...
$\mu$                                                 & Mean of number density                    & 1.0 $\text{cm}^{-3}$                 & \citet{Wagner2012}              \\
