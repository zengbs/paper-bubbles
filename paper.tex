...
In this section, we present the simulation results (Fig. \ref{fig__slice-compare})\
...
The parameters of jet models, including the fiducial run (C),\
covered in this appendix are given in Table \ref{tablb-jet-parameters}.

Three points are worth highlighting:
\begin{itemize}
   \item Increase density only (F$\rightarrow$C$\rightarrow$E):\\
   The runs F, C, and E demonstrate that an increase in density,
   causes a rise in the intensity of the shocks driven into the atmosphere,\
   and makes the post-shocked region hotter,\
   as well as leads the bubbles to be more asymmetric about the disk.
   Since the jets with higher denisty are more likely to\
   break through the thin disk in a short time\
   and more difficult to be elongated by stratified atmosphere afterward.

   \item Increase velocity only (G$\rightarrow$C$\rightarrow$A):\\
   Increasing magnitude of velocity also gives the hotter and asymmetric bubbles as increasing density.
   The temperature of the bubbles caused by increasing velocity\
   is closely similar to that of increasing density when kinetic energy is fixed.\

   \item Increase thermal energy only (B$\rightarrow$C$\rightarrow$D):\\
   These demonstrates that increasing thermal energy cannot affect\
   the morphology of bubbles significantly
\end{itemize}

   This shows that the morphology/dynamic of bubbles is mainly determined by\
   a kinetic energy of a jet flow. Jets with higher kinetic energy are more likely to\
   break through the thin disk in a short time\
   and more difficult to be elongated by stratified atmosphere afterward.
...
\begin{figure*}
  ...
  \caption
   {
    Simulation results based on the jets parameters in Fig. \ref{fig__jet_parameters}.
   }
