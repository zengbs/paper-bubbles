...
\subsection{Assumptions and Numerical Techniques}
  ...
  (Schive et al. \citeyear{gamer-1}, \citeyear{gamer-2}; \citeauthor{tseng2021} \citeyear{tseng2021}) to carry out\
  the simulations of Fermi and eROSITA bubbles. \textsc{gamer-sr} adopts a new algorithm \citep{tseng2021} to convert between\
  primitive and conserved variables, significantly reducing numerical error caused by catastrophic cancellations\
  that commonly occur within the regions with high Mach number. i.e. jet-ISM interaction zones.
  ...
  We treat CRs as a second fluid and solve directly for the evolution of CR pressure $p_{\text{cr}}$\
  as a function of $\mathbf{r}$ and $t$.\
  We did not model the CR energy spectrum and neglect the cooling and heating processes of CRs,\
  ...
  including CR advection, and dynamical coupling between the thermal gas and CRs. The governing equations\
  can be written as
...
\begin{equation}
  \frac{dE}{dtd\epsilon_{1}dV} = \frac{3}{4}\sigma_{T}c\textit{C}\int^{\epsilon_{\text{max}}}_{\epsilon_{\text{min}}}
               \epsilon_{1}\frac{n(\epsilon)}{\epsilon}d\epsilon\int^{\gamma_{\text{max}}}_{\gamma_{\text{min}}}
               \gamma^{-(p+2)}f(q, \Gamma)d\gamma.
\end{equation}


\begin{equation}
f(q, \Gamma) = 2q\ln q+(1+2q)(1-q)+0.5(1-q)\frac{\left(\Gamma q\right)^2}{1+\Gamma q},
\end{equation}
...
\begin{equation}
\sigma(\epsilon_{\text{p}})=34.3+1.88L+0.25L^{2} \text{ mb}.
\end{equation}
...
where $B=1.30+0.14L+0.011L^2$, $\beta=\left(1.79+0.11L+0.008L^{2}\right)^{-1}$,\
