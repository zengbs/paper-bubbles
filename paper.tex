...
\section{Methodology}
  ...
  by an plane-parallel potential that is symmetric about the mid-plane $z=0$\
  ...
  \begin{enumerate}
     \item  We multiply $f(\bold{x})$ and $\rho_{\text{isoDisk}}(z)$\
            to obtain the density of disk $\tilde{\rho}_{\text{ismDisk}}(\bold{x})$. i.e.\
            $\displaystyle\tilde{\rho}_{\text{ismDisk}}(\bold{x}) =\
            f(\bold{x}) \rho_{\text{isoDisk}}(z)$.\
            The temperature is determined by
            $\displaystyle \tilde{T}_{\text{ismDisk}} = T_{\text{isoDisk}}\rho_{\text{isoDisk}}/\tilde{\rho}_{\text{ismDisk}}$.
     \item  If $\tilde{T}_{\text{ismDisk}}$ larger than $T_{\text{crit}}$,
  \end{enumerate}
  ...
  The porosity of the disk arises by imposing an upper temperature cutoff for the\
  existence of clouds at $T_{\text{crit}} = 10^{4}$ K, beyond which clouds
  are deemed thermally unstable and replaced by temperature of atmosphere.

  The temperature in each cell within the disk is determined by pressure equilibrium with hot gas.\
  The clouds within the disk are in pressure equilibrium with the surrounding hot voids, thus\
  $\displaystyle T_{\text{ismDisk}} = T_{\text{isoDisk}}\rho_{\text{isoDisk}}/\rho_{\text{ismDisk}}$.
...
%  \begin{enumerate}
%     \item Constructing the isothermal disk $\rho_{\text{isoDisk}}(z)$ by \Cref{isothermal-disk-density}.
%     \item Using PyFC code to obtain a dimensionless clumpy distribution $f$ obeying\
%           lognormal distribution and the power-law spectrum, \Cref{Kolmogorov-spectrum}.
%     \item \Cref{density-ism-disk}.
%     \item
%  \end{enumerate}
...
$\dagger$  $k_{\text{min}}$           & Cutoff wave number                        & 375.0                                &   N/A                          \\
$\mu$                                 & Mean of scalar field                      & 1.0                                  &   N/A                          \\
$\ddag$  $\sigma$                     & Dispersion of scalar field                & 2.2                                  & \citep{Federrath2010}          \\
$\beta$                               & Power law index                           & -5/3                                 &   N/A                          \\ \midrule
\end{tabular}
\begin{tablenotes}
      \raggedright
      \item  $\dagger$  $k_{\text{min}}=375.0$ leads to the size of an individual clump $\sim 100$ pc.
      \item    $\ddag$  $\sigma=$ ....
    \end{tablenotes}
\end{table*}
