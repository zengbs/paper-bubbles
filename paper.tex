...
\subsection{Inclined jet injection}
  ...
  $8\times10^{55}$ -- $1.3\times10^{56}$ erg, estimated by \citet{Predehl2020}.\

  %, and prevent the disk from destruction by .
  % ablate
...
\subsection{Morphology}
 ...
 % the innermost bubbles is a reverse shock
 % the forward shock
...
even simulated microwave spectra are marginally consistent with the observed.
...
\item The Galactic bubbles are nearly symmetric about the Galactic plane even the jet is at an angle $45^{\circ}$\
...
\item The edge of the eROSITA bubbles is a forward shock,\
      originally driven by a oblique bipolar jets emanating from the GC 12.39 million years ago,\
      and significantly stretched by the stratified ambient afterwards.
\item Followed by the forward shock is a contact discontinuity,\
      which we interpret as the edge of the \textit{Fermi} bubbles\
      The nature of the \textit{Fermi} bubbles are essentially a turbulent and high-temperature ($\sim1$ keV)\
      plasma in pressure balance with the external medium. Associated with the Galactic magnetic filed may\
      lead to stochastic acceleration of CRe, and balanced with radiative cooling.\
...
The broad agreement between simulated and observed X-ray map provides supporting\
evidence for the inclined jet scenario,
